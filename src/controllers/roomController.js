...
class roomController {
    ...
    static async roomById(req,res){
        ...
        //check get by id
        await roomModel.where({id: room_id}).fetch()
            .then(data=>{
                if(!data) return res.status(400).json({status:'error', error: "room with id not found"})
    ...
    static async create(req,res){
        ...
        if( !roomtype_id || !room_name || !description ){
            return res.status(400).json({status:'error', error: "All fields are required to register room"})
        }
    ...
    static async update(req,res){
        ...
        const { roomtype_id, room_name, description, available, condition } = req.body;
        ...
        await roomModel
            ...
            .fetch()
            .then(room=>{
                if(!room) return res.status(400).json({status:'error', error: "room with id not found"})
                // room = room.toJSON()
                room.set(roomData)
                room.save().then(savedroom =>{
                    savedroom = savedroom.toJSON()
                    if(savedroom) return res.status(200).json({
                        status: 'success',
                        data: savedroom
                    })
                })
    ...
    static async delete(req,res){
        ...
        //check get by id
        await roomModel.forge({id: room_id}).fetch()
            .then(room=>{
                if(!room) res.status(400).json({status:'error', error: "room with id not found"})
                room.destroy()
                    .then(()=>{
                        return res.status(200).json({status:'success', data: "Successfully deleted room"})
                    })
            })
