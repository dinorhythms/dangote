...
export async function guestRouteAuth(req, res, next) {
  ...
  if (!token) {
    ...
    try {
      ...
      res
        .status(401)
        .json({ status: 'error', error: 'Invalid token, authorization denied for route' });
    }
  }
}

export async function receptionistRouteAuth(req, res, next) {
  let token = req.headers.authorization;
    ...
    try {
      ...
      await userModel
        ...
        .then((data) => {
          ...
          if (user.role_id !== 3 && user.role_id !== 6) return res.status(401).json({ status: 'error', error: 'Authorization denied' });
...
export async function serviceRouteAuth(req, res, next) {
  let token = req.headers.authorization;
    ...
    try {
      ...
      await userModel
        ...
        .then((data) => {
          ...
          if (user.role_id !== 2 && user.role_id !== 6) return res.status(401).json({ status: 'error', error: 'Authorization denied' });
